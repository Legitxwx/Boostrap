...
local function buildWindow()
    local UI = ensureWindUI(); if not UI then warn("Legitxwx Load"); return end
    local Window = UI:CreateWindow({
        Title = "Fish It",
        Icon = "circle-check",
        Author = "Boostrap Community",
    ...
    Window:EditOpenButton({ Title = "Open - Fish It", Icon = "circle-check", CornerRadius = UDim.new(0,16), StrokeThickness = 2, Color = ColorSequence.new(Color3.fromHex("9600FF"), Color3.fromHex("AEBAF8")), Draggable = true })
    Window:Tag({ Title = "V1.0 STABLE", Color = Color3.fromHex("#ffcc00") })
    UI:SetNotificationLower(true)
    pcall(function() UI:Notify({ Title = "Success", Content = "Script loaded successfully", Duration = 4, Icon = "circle-check" }) end)

    local Dev = Window:Tab({ Title = "Home Info", Icon = "hard-drive" })
    local MainFeautures = Window:Tab({ Title = "Main", Icon = "toggle-right" })
    ...
    local Teleport = Window:Tab({ Title = "TP MAP", Icon = "map" })
    ...
    local inviteData = LookupDiscordInvite("XsfetqqGkN")
    if inviteData then
        Dev:Paragraph({
    ...
    Dev:Paragraph({ Title = "Credits", Desc = "UI: WindUi/nDev: Legitxwx", Locked = true })
    ...
    task.spawn(function()
        while task.wait(config.weather.pollIntervalSeconds) do
            if not state.AutoBuyWeather then continue
                end
    ...
    local function startInfiniteOxygen()
        ...
        if typeof(hookmetamethod) == "function" and typeof(newcclosure) == "function" and typeof(getnamecallmethod) == "function" then
            if not _G.__Legit_OXY_HOOK then
                local old
                old = hookmetamethod(game, "__namecall", newcclosure(function(self, ...)
                    local method = getnamecallmethod()
                    if _G.__Legit_OXY_BLOCK and method == "FireServer" and tostring(self) == "URE/UpdateOxygen" then
                        return nil
                    end
                    return old(self, ...)
                end))
                _G.__Legit_OXY_HOOK = true
            end
            _G._Legit__OXY_BLOCK = true
        end
        -- Respawn listener
        if not oxyConn then
    ...
    task.spawn(function()
        while task.wait(5) do
            ...
            if targetEventPart and not eventState.isAtEvent then
                ...
                eventState.platform.Name = "EventPlatform"
    ...
    local function getConfigKey()
        return "Legit_Config_" .. tostring(player.UserId or player.Name or "User")
    end
    ...
    local function sendFishWebhook(fishName, rarityText, assetId, itemId, variantId)
        ...
        local embed = {
            ...
            ["image"] = { ["url"] = imageUrl },
            ["footer"] = { ["text"] = "Legitxwx Webhook | " .. os.date("%H:%M:%S") }
        }
        if fields then embed["fields"] = fields end
        local data = { ["username"] = "Fisher - Notification System", ["embeds"] = { embed } }
    ...
    Webhooksettings:Button({
        ...
        Callback = function()
            ...
            local WebhookURL = "https://discord.com/api/webhooks/" .. webhookState.webhookPath
            local data = {
                ["username"] = "Fisher - Notification System",
                ["embeds"] = {{
                    ["title"] = "ðŸ§ª Test Webhook",
                    ["description"] = "This is a test message from Legitxwx script!",
                    ["color"] = tonumber("0x00ff00"),
                    ["footer"] = { ["text"] = " Webhook Test | " .. os.date("%H:%M:%S") }
                }}
            }
